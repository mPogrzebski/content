...
It builds on promises, e.g. `doSomething()` is the same function as before. You
...
{{jsxref("Promise.all()")}} and {{jsxref("Promise.race()")}}
...
Note that the optional steps here are nested, not from the indentation, but from
